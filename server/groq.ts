...
const PERFUME_SYSTEM_PROMPT_AR = `أنت "لمى" — المستشارة الشخصية لمتجر RF Perfume للعبايات الفاخرة، خبيرة في الموضى والأزياء النسائية الراقية.
...
const PERFUME_SYSTEM_PROMPT_EN = `You are "Lama" — the personal style advisor of RF Perfume, a luxury abaya brand based in Riyadh.
...
export function smartAdvisorFallback(
  ...
  const response = lang === "ar"
    ? (refs.length > 0
        ? `بناءً على ما ذكرت، أرشّح لك من تشكيلة RF Perfume: ${names} ✨\nهذه اختيارات مثالية وتحظى بإعجاب عملائنا. اضغط على أي منها لمعرفة التفاصيل أو إضافته للسلة مباشرة.`
        : "أهلاً بك في RF Perfume ✨ أخبرني أكثر عن ذوقك (هل تفضل العود، الورد، المسك؟ للنهار أم الليل؟) وسأرشّح لك العطر المثالي.")
    : (refs.length > 0
...
const SUPPORT_SYSTEM_PROMPT_AR = `أنت "آر اف" — مساعد الدعم الفني لمتجر RF Perfume للعطور والعود الفاخر.
...
const SUPPORT_SYSTEM_PROMPT_EN = `You are "RF" — the support assistant for RF Perfume, a luxury perfume & oud brand.
...
export async function adminAssistant(
  ...
  const systemMsg = lang === "ar"
    ? `أنت "آر اف" — مساعد الإدارة الذكي لمتجر RF Perfume للعطور والعود الفاخر.
...
${context?.role ? `\n**دور المستخدم:** ${context.role}` : ""}`
    : `You are "RF" — the smart management assistant for RF Perfume, a luxury perfume & oud brand.
