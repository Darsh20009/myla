...
export function smartAdvisorFallback(
  ...
  const response = lang === "ar"
    ? (refs.length > 0
        ...
        : "أهلاً بك في Myla ✨ أخبرني أكثر عن ذوقك (هل تفضلين العباءات الكلاسيكية، المطرّزة، الكاجوال؟ لأي مناسبة؟) وسأرشّح لك القطعة المثالية.")
    : (refs.length > 0
        ...
        : "Welcome to Myla ✨ Tell me more about your style (do you prefer classic, embroidered, or casual abayas? For what occasion?) and I'll suggest the perfect piece.");
...
const SUPPORT_SYSTEM_PROMPT_AR = `أنت "ميلا" — مساعد الدعم الفني لمتجر Myla للعبايات الفاخرة.
...
const SUPPORT_SYSTEM_PROMPT_EN = `You are "Myla" — the support assistant for Myla, a luxury abaya & women's fashion brand.
...
export async function adminAssistant(
  ...
  const systemMsg = lang === "ar"
    ? `أنت "ميلا" — مساعد الإدارة الذكي لمتجر Myla للعبايات والأزياء النسائية الفاخرة.
...
${context?.role ? `\n**دور المستخدم:** ${context.role}` : ""}`
    : `You are "Myla" — the smart management assistant for Myla, a luxury abaya & women's fashion brand.
