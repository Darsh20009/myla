...
export function SizeAdvisor({ productName, productCategory, availableSizes, availableLengths = [], onSizeSelect, onLengthSelect }: SizeAdvisorProps) {
                ...
                {result && !result.error && (
                  ...
                  >
                    ...
                    {result.recommendedLength && (
                      <div className="bg-primary/5 border border-primary/15 p-3 space-y-2">
                        <div className="flex items-center gap-2">
                          <Ruler className="h-3.5 w-3.5 text-primary" />
                          <p className="font-black text-xs">الطول الموصى به: <span className="text-primary">{result.recommendedLength}</span></p>
                        </div>
                        {result.lengthReasoning && (
                          <p className="text-[10px] text-black/55 font-bold leading-relaxed">{result.lengthReasoning}</p>
                        )}
                        {onLengthSelect && availableLengths.includes(result.recommendedLength) && (
                          <Button
                            size="sm"
                            variant="outline"
                          ...
                          >
                            اختر الطول {result.recommendedLength}
                          </Button>
                        )}
                      </div>
                    )}
