...
| `APPLE_REDIRECT_URI` | Apple OAuth callback URL | For Apple auth |
...
| `STORAGE_STATION_API_SECRET` | WooCommerce consumer secret | For 3PL |
| `SHIPOX_USERNAME` | Shipox / 3rd Mile login email | For shipping |
| `SHIPOX_PASSWORD` | Shipox / 3rd Mile login password | For shipping |
| `SHIPOX_BASE_URL` | Override Shipox URL (default: `https://3rdmile.my.shipox.com`) | Optional |
| `PAYMOB_API_KEY`, `PAYMOB_SECRET_KEY`, `PAYMOB_PUBLIC_KEY`, `PAYMOB_HMAC_SECRET`, `PAYMOB_INTEGRATION_ID` | Paymob payment gateway | For payments |
| `TABBY_SECRET_KEY`, `TABBY_PUBLIC_KEY`, `TABBY_WEBHOOK_SECRET`, `TABBY_MERCHANT_CODE` | Tabby BNPL | For payments |
| `TAMARA_API_TOKEN`, `TAMARA_NOTIFICATION_TOKEN`, `TAMARA_PUBLIC_KEY` | Tamara BNPL | For payments |

> **Note:** `GEMINI_API_KEY`, `GEMINI_API_KEY_2`, `GEMINI_API_KEY_3` are optional and unused — Kimi is the sole AI provider.
